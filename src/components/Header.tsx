      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-6">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          <Link to="/" className={navLink} activeOptions={{ exact: true }} activeProps={{ className: "text-primary font-semibold" }}>
            ...
            >
              <button
                className={`${navLink} flex items-center gap-1 focus:outline-none`}
              >
              ...
              {destOpen && (
                <div className="absolute left-0 top-full w-64 pt-2">
                  <div className="rounded-lg border border-border bg-card p-2 shadow-luxe">
                    ...
                    <Link
                      to="/destinos"
                      className="mt-1 block rounded-md border-t border-border px-3 pt-2 pb-1 text-sm font-bold text-primary hover:bg-secondary"
                      onClick={() => setDestOpen(false)}
                    >
          ...
          <div className="border-t border-border bg-background lg:hidden">
            <nav className="flex flex-col gap-1 p-4">
              ...
              <div className="flex flex-col gap-1 pl-4 py-2">
                ...
                <Link to="/destinos" className="mt-1 rounded-md border-t border-border px-3 pt-2 text-sm font-bold text-primary hover:bg-secondary" onClick={() => setOpen(false)}>{t("Ver todos los destinos →", "View all destinations →")}</Link>
              </div>
